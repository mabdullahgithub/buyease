...
const formFieldSchema = z.object({
  ...
  prefixText: z.string().max(50).optional(),
...
export const formDesignConfigSchema = z.object({
  ...
  formBorderWidthPx: z.number().int().min(0).max(10).default(1),
  formShadowPx: z.number().int().min(0).max(40).default(8),
  formPaddingPx: z.number().int().min(0).max(60).default(24),
  formTextBold: z.boolean().default(false),
  formTextItalic: z.boolean().default(false),
...
export const formSettingsConfigSchema = z.object({
  ...
  allowedCountries: z.array(z.string().length(2)).max(250).default([]),
  enableOrderEligibility: z.boolean().default(false),
  orderEligibilityMin: z.number().min(0).nullable().default(null),
  orderEligibilityMax: z.number().min(0).nullable().default(null),
  showIneligibleMessage: z.boolean().default(false),
  ineligibleMessage: z.string().max(500).default(""),
